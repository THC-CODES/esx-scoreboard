...
function UpdatePlayerTable(connectedPlayers)
	...
	local ems, police, taxi, mechanic, unemployed, cardealer, estate, players = 0, 0, 0, 0, 0, 0, 0, 0
	...
	SendNUIMessage({
		...
		jobs   = {ems = ems, police = police, taxi = taxi, mechanic = mechanic, unemployed = unemployed, cardealer = cardealer, player_count = players}
	})
end
...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlJustReleased(0, Keys['F3']) and IsInputDisabled(0) then -- Change `F3` to the Keybind you'd like
			ToggleScoreBoard()
			Citizen.Wait(200)
